...
const ManagerDashboard = () => {
  ...
  const GetProducts = async () => {
    try {
      const response = await axios.get(
        'http://localhost:3000/manager/getAllProduct',
        {
          headers: {
  ...
  const handleDeleteClick = async (productId) => {
    ...
    try {
      const respons = await axios.delete(`http://localhost:3000/manager/deleteProduct/${productId}`);
    ...
    allProducts.map((product) => (
      <tr key={product.productId}>
        ...
        <td className="py-2">
          ...
          <img
            src={`http://localhost:3000/manager/getproductimage/${product.picture}`}
            alt={product.name}
